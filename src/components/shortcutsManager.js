...
export class ShortcutsManager {
	...
	calculateItemsPerPage() {
		...
		const container = this.elements.textShortcutsList || this.elements.morseShortcutsList;
		const list = this.elements.textShortcutsList || this.elements.morseShortcutsList;
		if (!modalBody || !container || !list) return 5;

		const listHeader = container.querySelector('.shortcuts-list-header');
		const addShortcut = container.querySelector('.add-shortcut');

		const modalHeight = modalBody.offsetHeight || window.innerHeight * 0.95;

		const listHeaderHeight = listHeader?.offsetHeight || 0;
		const addShortcutHeight = addShortcut?.offsetHeight || 0;
		...
		const sampleItem = list.querySelector('.shortcut-item') || list.firstElementChild;
		const itemHeight = sampleItem?.offsetHeight || 50;

		const availableHeight = modalHeight - listHeaderHeight - addShortcutHeight - padding - 40;
